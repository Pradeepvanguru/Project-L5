  ...
  const emailMessage = `
    <div style="display:flex;align-items:center;justify-content:center; color:white; background-color:grey; border:2px solid yellow;  border-radius:3rem;">
      <div style="padding:20px;margin:15px;">
        ...
        <p>
          <a href="https://project-l5.onrender.com/api/leave/accept/${leaveRequest._id}?acceptingEmail={{userEmail}}&priority=${priority}" style="padding: 14px; background-color: green; color: white;font-size:18px; decoration:none;">Accept</a>
          <a href="https://project-l5.onrender.com/api/leave/reject/${leaveRequest._id}?rejectingEmail={{userEmail}}" style="padding: 14px; background-color: red; color: white;font-size:18px; decoration:none;">Reject</a>
        </p>
      </div>
    </div>
  `;
            ...
            setTimeout(async () => {
              ...
              if (leaveStatus && leaveStatus.status === 'Pending') {
                ...
                await sendEmail(leaveRequest.email, 'No Response to Leave Request', `
                  <div style="align-items:center; color:black; background-color:yellow; padding:10px;border-radius:2rem; ">
                    <h1>No one accepted your leave Request....Try another Day..!</h1>
...
router.get('/accept/:id', async (req, res) => {
  ...
  try {
    ...
    if (!leaveRequest) return res.status(404).send('<h3>This Request mail has expired. You are too late!</h3>');

    // Check if the request has expired
    if (await checkEmailExpiration(leaveRequest)) {
      return res.send('<h1>This email has expired.</h1>');
    }
    ...
    if (parseInt(priority) === 1) {
      ...
      // // Set timeout to delete the pending state from leaveRequest after 2 minutes
      ...
      // Calculate the time for 3 days in milliseconds
      setTimeout(async () => {
        try {
          ...
          // Query to delete accepted records where the leaveDate is 3 days ago
          await LeaveRequest.deleteMany({
            status: 'Accepted',
            leaveDate: { $lte: threeDaysAgo } // Match leaveDate that is exactly or more than 3 days ago
          });
      
          console.log('Deleted accepted records where leaveDate is 3 days ago.');
        } catch (error) {
          console.error('Error deleting accepted records:', error);
        }
...
//   // Set the time to 11:59:00 PM today
//   midnight.setHours(23, 59, 0, 0);
  
//   // If the current time is already past 11:59 PM, set the target to 11:59 PM the next day
//   if (now > midnight) {
//     midnight.setDate(midnight.getDate() + 1);
//   }

//   return midnight - now; // Time in milliseconds until 11:59 PM
// }
// setTimeout(async () => {
//   try {
//     await LeaveRequest.deleteMany({ status: 'Pending' });
//     console.log('Deleted pending records after 11:59 PM.');
//   } catch (error) {
//     console.error('Error deleting pending records:', error);
//   }
// }, getTimeUntilMidnight());

      // Set timeout to delete the accepted state from leaveRequest after 3 minutes
     // 3 minutes

      // Function to calculate the time in milliseconds for 3 days
// const threeDaysInMilliseconds = 3 * 24 * 60 * 60 * 1000;

// Set timeout to delete accepted records after 3 days
// setTimeout(async () => {
//   try {
//     await LeaveRequest.deleteMany({ status: 'Accepted' });
//     console.log('Disappear the accepted records after 3 days.');
//   } catch (error) {
//     console.error('Error deleting accepted records:', error);
//   }
// }, threeDaysInMilliseconds);
    ...
    res.send('<h1 style="align-items:center;display:flex; justify-content:center; color:white; background-color:green; padding:10px;  border-radius:3rem;">You are accepted the ClassAdjustment-request successfully...</h1>');
...
router.get('/reject/:id', async (req, res) => {
  try {
    ...
    res.send('<h1 style="align-items:center; color:white; background-color:red;display:flex; justify-content:center; padding:10px; ">Your rejected the request successfully.</h1>');
  } catch (error) {
